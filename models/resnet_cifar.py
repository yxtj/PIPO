...
from src.model.dag_model import AddOp, DagModel
...
def build_resnet(num_blocks, num_class=100, version=1, residual=True, batch_norm=False):
    ...
    if version == 1: # cifar10
        ...
        raise ValueError("version id is not valid")
    if not residual:
        layers = [ lyr for lyr in layers if not isinstance(lyr, tuple) or not isinstance(lyr[0], AddOp) ]
    #layers.append(nn.Softmax(dim=1))
    return DagModel(*layers)
